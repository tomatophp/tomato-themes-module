    ...
    <x-slot:buttons>
        <div class="flex justify-end">
            <x-splade-form class="mx-2" action="{{route('admin.themes.index')}}" method="GET">
                <x-splade-input @change="form.submit()" placeholder="{{ __('Search By Theme Name') }}" name="search" type="search"></x-splade-input>
            </x-splade-form>
        ...
        <x-splade-data default="{show: false}">
            <div class="grid grid-cols-2 gap-4">
                @foreach($themes as $theme)
                    <div>
                        <div class="border border-gray-300 bg-center bg-cover w-full h-60" style="background-image: url('{{ str($theme['info']->placeholder)->contains('http') ? $theme['info']->placeholder :theme_assets('images/'. $theme['info']->placeholder)}}')"></div>
                        @if(setting('theme_name') === $theme['info']->name)
                            <div class="flex justify-between bg-gray-900 text-white">
                ...
                <Link href="{{route('admin.themes.create')}}" class="h-[300px] border border-dashed hover:border-solid hover:border-primary-500 border-4 border-gray-200 hover:border-none hover:bg-primary-500 text-gray-300 hover:text-white">
